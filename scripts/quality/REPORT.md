...
- **Problem:** After successful `git commit`, real-index restore (`_set_index_state` post_index) and checkpoint append are outside a single failure boundary; unexpected paths raise _after_ HEAD moved.
...
See [COMPARISON.md](COMPARISON.md) for the 2026-08-03 post-repair re-scan vs the original hotspot baseline (CodeScene + mechanical deltas).
